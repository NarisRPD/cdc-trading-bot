...
| `TOP_PICKS_N` | `10` | จำนวนหุ้นสูงสุดใน `/top10` (ตลาดแคบได้น้อยกว่านี้ได้) |
| `ALLOC_SCORE_TILT` | `0.5` | สัดส่วนแบ่งเงินเอียงตามคะแนนแค่ไหน · `0` = ถ่วงด้วยความเสี่ยงล้วน |
| `MAX_PER_SECTOR` | `2` | จำกัดหุ้นต่อเซกเตอร์ใน `/top10` · `1` = เข้มสุด · `0` = ปิด (เอาตามคะแนนล้วน) |
| `MAX_CORR` | `0.75` | ตัดหุ้นที่ราคาวิ่งตามตัวที่เลือกแล้ว (corr 60 วัน) — กันป้ายเซกเตอร์หลอก |
| `MIN_DOLLAR_VOL_M` | `5` | มูลค่าซื้อขายขั้นต่ำ (ล้าน$/วัน) — กรองหุ้นสภาพคล่องต่ำ |
| `MCAP_MIN_M` / `MCAP_MAX_M` | `300` / `100000` | ขนาดบริษัท (ล้าน$) — default ตัด mega cap ออก |
| `GROWTH_MIN_REV_G` | `20` | ยังขาดทุนต้องรายได้โตอย่างน้อยกี่ % ถึงผ่านด่านพื้นฐาน |
| `MIN_ATR_PCT` | `1.2` | หุ้นต้องแกว่งอย่างน้อยกี่ %/วัน — ตัดหุ้นติดดีลซื้อกิจการ/ราคาถูกตรึง (ไม่มีเทรนด์ให้รัน) |
| `TOP_BUDGET_USD` | `1000` | งบรวมของก้อน `/top10` — แปลง % เป็นเงินจริงต่อตัว (≈ $XXX) |
| `PREFER_SECTORS` | (ว่าง) | เซกเตอร์โปรด บวกคะแนนพิเศษ เช่น `technology` หรือ `เทคโนโลยี` (คั่นด้วย ,) |
| `PREFER_SECTOR_BONUS` | `5` | เซกเตอร์โปรดได้บวกกี่คะแนน (บูสต์เบา ๆ ไม่ใช่บังคับเลือก) |
