...
def spike_flags(s, *, min_dollar_vol_m: float = 5.0) -> "list[str]":
    ...
    if up is not None:
        if up >= 18.0:
            # วันเดียว +18% = เกือบแน่ว่าเป็นข่าว/งบ/สัญญา — เทรนด์แท้ไม่ขึ้นแบบนี้
            out.append(f"พุ่งวันเดียว +{up:.0f}% ใน 20 วัน (ข่าว/สัญญา ไม่ใช่เทรนด์)")
        elif up >= 9.0 and r20 is not None and r20 > 0 and up >= 0.7 * r20:
