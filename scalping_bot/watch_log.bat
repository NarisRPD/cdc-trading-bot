...
echo  Path: %~dp0scalpbot.log
echo ============================================================
echo.
rem -Encoding UTF8 จำเป็น: log เป็น UTF-8 ไม่มี BOM — PS 5.1 ไม่ใส่จะอ่านเป็น ANSI = ไทยมั่ว
powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-Content -Path '%~dp0scalpbot.log' -Wait -Tail 80 -Encoding UTF8"
