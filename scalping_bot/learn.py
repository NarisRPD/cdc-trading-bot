"""
scalping_bot/learn.py — วงจรการเรียนรู้จากผลเทรดจริง (closed-loop)
...
_FILE = os.path.join(os.path.dirname(__file__), "scalpbot_trades.json")
_MAGIC = 260605
...
def export(fmt: str = "csv") -> "str | None":
    """ส่งออก scalpbot_trades.json → ไฟล์ CSV หรือ JSONL สำหรับเทรน ML/fine-tune ภายนอก
    ...
    if fmt not in ("csv", "jsonl"):
        fmt = "csv"
    out_path = os.path.join(os.path.dirname(__file__), f"scalpbot_trades_export.{fmt}")
    if fmt == "jsonl":
