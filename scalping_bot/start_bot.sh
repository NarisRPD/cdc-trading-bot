...
while [ -f "$FLAG" ]; do
    # ดึงโค้ดใหม่จาก GitHub ก่อน start ทุกครั้ง (รวมตอน /restart จาก Telegram)
    # --ff-only: ปลอดภัย — ไม่ merge ถ้ามี conflict (fail loud แทน silent overwrite)
    echo "[$(date '+%F %T')] git pull ..." | tee -a "$SCRIPT_DIR/part2.log"
    git -C "$SCRIPT_DIR" pull --ff-only >> "$SCRIPT_DIR/part2.log" 2>&1 || \
        echo "[$(date '+%F %T')] ⚠️ git pull ล้มเหลว — ใช้โค้ดเดิม" | tee -a "$SCRIPT_DIR/part2.log"
