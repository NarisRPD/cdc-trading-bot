"""
scalping_bot/interactive.py — โหมดโต้ตอบ: ส่งใบสั่ง + ปุ่มกดใน Telegram
...
# ตัดทุกเที่ยงคืน → scalpbot.log (วันนี้) + scalpbot.log.YYYY-MM-DD (N วันก่อน)
# จำนวนวันปรับได้ผ่าน env PART2_LOG_DAYS (ดีฟอลต์ 7) — เพิ่มถ้าอยากย้อนดูเหตุการณ์เก่า
# *** .bat ต้องรัน `python interactive.py` เฉย ๆ (ห้าม >> scalpbot.log 2>&1) ไม่งั้นเขียนซ้ำ/rotate พัง ***
from logging.handlers import TimedRotatingFileHandler

# ── ย้ายบ้านชื่อไฟล์: part2_* → scalpbot_* (rebrand เป็น Scalping Bot Trade by narisrpd) ──
# รันครั้งเดียวตอนบูต: เจอไฟล์ชื่อเก่าและยังไม่มีชื่อใหม่ → เปลี่ยนชื่อให้อัตโนมัติ
# ความจำบอท (ประวัติเทรด/cooldown/shadow/journal/log) ติดมาครบ ไม่ต้องย้ายมือ
def _migrate_part2_names() -> None:
    import glob as _glob
    _base = os.path.dirname(os.path.abspath(__file__))
    _ld = (os.getenv("SCALPBOT_LOG_DIR") or os.getenv("PART2_LOG_DIR") or "").strip()
    for d in [_base] + ([_ld] if _ld and os.path.isdir(_ld) else []):
        for _old in _glob.glob(os.path.join(d, "part2*")):
            _new = os.path.join(d, os.path.basename(_old)
                                .replace("part2_", "scalpbot_", 1).replace("part2.", "scalpbot.", 1))
            try:
                if _new != _old and not os.path.exists(_new):
                    os.replace(_old, _new)
            except OSError:
                pass


_migrate_part2_names()

# path ของ log ตั้งค่าได้ผ่าน env SCALPBOT_LOG_DIR (ชื่อเก่า PART2_LOG_DIR ยังใช้ได้) →
# ชี้ไปโฟลเดอร์ OneDrive/Drive ให้ cloud sync อัตโนมัติ · ดีฟอลต์ = โฟลเดอร์ script
_LOG_DIR = ((os.getenv("SCALPBOT_LOG_DIR") or os.getenv("PART2_LOG_DIR") or "").strip()
            or os.path.dirname(os.path.abspath(__file__)))
try:
    os.makedirs(_LOG_DIR, exist_ok=True)
except OSError:
    _LOG_DIR = os.path.dirname(os.path.abspath(__file__))   # path เสีย → fallback โฟลเดอร์ script
_LOG_FILE = os.path.join(_LOG_DIR, "scalpbot.log")
try:
...
#   = file handler ไม่ติด → scalpbot.log ว่างเปล่า (เคยเป็นบั๊กนี้)
...
# จับ exception ที่หลุด (uncaught) → เขียนลง scalpbot.log ด้วย (เดิมไปแค่ console → หาย ดู crash ไม่ได้)
# กันเคสบอท crash แล้ว scalpbot.log ไม่มีร่องรอยว่าพังเพราะอะไร
...
_TRADE_META = os.path.join(os.path.dirname(__file__), "scalpbot_trade_meta.json")
...
_PAUSE = __import__("os").path.join(__import__("os").path.dirname(__file__), "scalpbot_paused.flag")
_SHOULD_RUN = __import__("os").path.join(__import__("os").path.dirname(__file__), "scalpbot_should_run.flag")
...
_STATE_FILE = __import__("os").path.join(__import__("os").path.dirname(__file__), "scalpbot_state.json")
...
_LOCK = __import__("os").path.join(__import__("os").path.dirname(__file__), "scalpbot.lock")
...
def main():
    ...
    def _touch_recent(key: str, ts: float) -> None:
        """อัปเดต cooldown + persist ลง scalpbot_state.json ทันที (ผ่าน restart ได้)"""
        recent[key] = ts
    ...
    while True:
        try:
            if not os.path.exists(_SHOULD_RUN):       # กดปิดบอท (ลบ flag) → ปิดอย่างปลอดภัย
                log.info("🛑 หยุดบอท (ไม่มี flag scalpbot_should_run — กดปิดจาก Stop)")
                break
            now = time.time()
            ...
            if slot_free and not blocked:
                ...
                if blackout:
                    ...
                    if auto_on:
                        ...
                        if scan_res:
                            # scan summary → log เสมอ (เพื่อ debug ใน scalpbot.log)
