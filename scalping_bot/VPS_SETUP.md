...
- ต้องมีทั้ง `scalping_bot\` **และ** `core\` (บอทเรียก `core.signals` ของ Part 1)
- วิธีก็อป: ZIP โฟลเดอร์ → ลากผ่าน RDP / Google Drive / GitHub

## 5) ตั้งค่า config.env บน VPS
แก้ `scalping_bot\config.env`:
...
2. เช็ก `scalpbot.log` ไม่มี error
...
- **หยุดบอท:** ลบไฟล์ `scalpbot_should_run.flag` (แต่ vps_run.bat สร้างใหม่ตอน logon — ถ้าอยากหยุดถาวรให้เอา shortcut ออกจาก Startup)
