"""
scalping_bot/journal.py — บันทึกผลเทรดของ Scalping Bot + สถิติ + P/L วันนี้

อ่านจาก history ของ MT5 (เฉพาะดีล magic 260605) → บันทึกไม้ที่ปิดแล้วลง scalpbot_journal.json
...
_FILE = os.path.join(os.path.dirname(__file__), "scalpbot_journal.json")
_MAGIC = 260605
...
def record_closed(days_back: int = 7) -> list:
    ...
    for d in deals:
        ...
        entry = {"deal_id": d.ticket, "symbol": d.symbol, "time": d.time,
                 ...
                 "position_id": d.position_id,            # ใช้ lookup ชื่อเทคนิคจาก scalpbot_trade_meta.json
